...
cat <<EOF
  ...
  Will create:
    ...
    - $target/scripts/*.sh                    (branch-hygiene, branch-start, check-secrets, check-todo-budget, seed-memory, test-backend, verify, hook-stop-verify)
...
Next steps:
  ...
  cd $target
  $hooks_step
  # Sanity check the Gradle wrapper (if present):
  ./gradlew --version
  git add .
  git commit -m "[CHORE]#0: bootstrap Claude Code workflow (backend)"
